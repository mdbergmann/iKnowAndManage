...
@implementation MBSearchOutlineView
...
{
	CocoLog(LEVEL_DEBUG,@"[MBSearchOutlineView -keyDown:] keycode: %d!",[theEvent keyCode]);
	
	// check for delete key code
	if([theEvent keyCode] == 51)
	{
		CocoLog(LEVEL_DEBUG,@"[MBSearchOutlineView -keyDown:] DELETE key pressed!");
...
- (IBAction)menuExport:(id)sender
{
	...
	{
		CocoLog(LEVEL_WARN,@"[MBSearchOutlineView -menuExport:] delegate does not repsond to selector!");
	}
}
