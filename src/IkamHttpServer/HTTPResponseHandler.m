...
+ (void)registerHandler:(Class)handlerClass {
	...
	int i;
	int count = [registeredHandlers count];
	for(i = 0; i < count; i++) {
...
+ (Class)handlerClassForRequest:(CFHTTPMessageRef)aRequest
    ...
    NSEnumerator *iter = [registeredHandlers objectEnumerator];
    Class handlerClass;
	while((handlerClass = [iter nextObject])) {
...
+ (HTTPResponseHandler *)handlerForRequest:(CFHTTPMessageRef)aRequest
	...
	NSDictionary *requestHeaderFields = [(NSDictionary *)CFHTTPMessageCopyAllHeaderFields(aRequest) autorelease];
	NSURL *requestURL = [(NSURL *)CFHTTPMessageCopyRequestURL(aRequest) autorelease];
	NSString *method = [(NSString *)CFHTTPMessageCopyRequestMethod(aRequest) autorelease];
	...
	HTTPResponseHandler *handler = [[[classForRequest alloc] initWithRequest:aRequest
                                                                      method:method
                                                                         url:requestURL
                                                                headerFields:requestHeaderFields
                                                                  fileHandle:requestFileHandle
                                                                      server:aServer] autorelease];
...
- (id)initWithRequest:(CFHTTPMessageRef)aRequest
	...
	self = [super init];
	if (self != nil) {
		request = (CFHTTPMessageRef)[(__bridge id)aRequest retain];
...
- (void)startResponse {
    ...
    // test response
    NSString *file = [[MBHTMLGenerator defaultGenerator] generateHTMLForItemList:[[MBItemBaseController standardController] currentItemSelection] 
                                                                     toOutputDir:nil options:nil];
	...
	CFHTTPMessageSetBody(response, (CFDataRef)[NSData dataWithContentsOfFile:file]);
    /*
